...
========================
The process performs fine-tuning for the complete EfficientNet-B0 model which includes both its backbone and head across all 19876 images.

Head-only training produces lower results than this method which proves to be better than head-only training
---------------------------
...
The head needs to adapt to the new backbone before any other components become active.

Phase B – Staged unfreeze (backbone unlocked one block at a time)
The last training block becomes active after unfreezing to create an evaluation period for stability.
The last two training blocks become active after unfreezing to create an evaluation period for stability.
The last three training blocks become active after unfreezing to create an evaluation period for stability.
The complete backbone system becomes active after unfreezing to create an evaluation period for stability.

The system keeps the backbone training rate at 10% of the head training rate during each training period.
The system maintains BatchNorm running statistics while only allowing affine parameters to update.
The system uses gradient clipping to maintain stable operation.

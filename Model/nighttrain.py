...
HEAD-ONLY MODELS EXIST BECAUSE BACKBONE FINE-TUNING CREATES CATASTROPHIC FORGETTING PROBLEMS:
The backbone fine-tuning process produces disastrous memory loss for the system when used with a small dataset of approximately 2000 visual samples.
The outcomes show that Run 1 head-only testing produced 83.13 percent results while Run 2 testing with backbone included achieved 16.87 percent results.
The system needs to use head training through multiple training sessions which will test different learning rate and noise levels until it reaches 86.86 percent achievement.

The system operates at a total cycle time of approximately seven minutes when it runs on CPU with all necessary features already stored in memory.
"""
...
# the 86.26% local optimum reached by the previous overnight run.
CONFIGS = [
...
def train_one_cycle(cfg, cycle_num):
    ...
    # Warm-start the classifier head only
    if BEST_PATH.exists():
        try:
...
while time_ok():
    ...
    # Save best model only if updated
    if improved:

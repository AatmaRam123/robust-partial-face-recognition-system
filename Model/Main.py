"""
You need to continue your work on fine-tuning with the original face subset which does not contain any additional features.

Reason For Using Original Data
Full training set has 18,462 images x 10 occlusion variants.
The complete set of images requires 20 to 30 minutes per epoch when processed through EfficientNet with its gradient capabilities.
The 'original' subset contains approximately 1978 images which require CPU processing time of 4 to 5 minutes per epoch.
The clean face images enable backbone adaptation through fine-tuning while Phase 1 head training provides pre-existing occlusion protection capabilities.
...
# Load only the classifier weights from checkpoint
